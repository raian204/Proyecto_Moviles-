...
export default function EntradaScreen({ navigation, productos, actualizarProductoFn, crearMovimientoFn }) {
  const [selected, setSelected] = useState(null);
  const [snack, setSnack] = useState({ visible: false, msg: "" });

  const handleGuardar = async ({ quantity }) => {
    if (!selected) return;
    ...
    try {
      ...
      setSelected(updated);
      setSnack({ visible: true, msg: "Entrada registrada" });
    } catch (e) {
